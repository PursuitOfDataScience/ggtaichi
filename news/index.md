...
- **Shared scales** for directly comparable sources (§4b of the
  roadmap):
...
- **Vectorized rendering**: each layer now draws all of its cells as one
  ...
  grids render an order of magnitude faster: a 1200-cell grid with eyes
  takes 0.24 s to build and draw versus 3.5 s in 0.2.0 (~15x, same
  machine). The drawn output is pixel-identical to 0.2.0 (verified on
  rotated, eyed, outlined, and faceted-discrete test plots).
