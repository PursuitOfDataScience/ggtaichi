...
- **`palette =`** on
  ...
  (a grey ramp and a hued ramp on the same luminance trajectory, so the
...
Found by auditing 0.3.0 against its own documentation before release.
None of these has appeared in a CRAN release, so all are corrections
rather than breaking changes.

- **The yin and yang legends could swap places between sessions.** Both
  auto-built fill guides were left at ggplot2’s default `order`, and the
  tie was broken by something that is not stable across R sessions: the
  same plot, the same package and the same ggplot2 could put yin first
  in one render and yang first in the next. One of the committed vdiffr
...
- **[`vignette("animations")`](https://pursuitofdatascience.github.io/ggtaichi/articles/animations.md)
  ...
  through
  [`gganimate::file_renderer()`](https://gganimate.com/reference/renderers.html),
  which needs no gifski and no system libraries, and emit a GIF on top
  of that wherever gifski exists. A demonstration that is never run is
  not a demonstration.
- **`palette = "print_safe"` is renamed `"greyscale_safe"`.** The preset
  guarantees that the two ramps collapse to the same ink in *greyscale*;
  it says nothing about the CMYK gamut, which is what most readers
  understand by “print safe”. The name over-promised. Renamed now
...
- **`explicit_channel = "radius"` gains `radius_exponent`, defaulting to
  ...
  behaviour.
- **[`taichi_check_palette()`](https://pursuitofdatascience.github.io/ggtaichi/reference/taichi_check_palette.md)
  now names the colour space it measured in.** Every number it prints is
  space-dependent and none of them said so, which made them impossible
  to check against another tool.
- **[`taichi_summary()`](https://pursuitofdatascience.github.io/ggtaichi/reference/taichi_summary.md)
  documents a caveat on `rank`.** The widest gap in a 96-cell grid is
  frequently the largest noise; `rank` lists places to look, not
  findings.
- **[`geom_taichi()`](https://pursuitofdatascience.github.io/ggtaichi/reference/geom_taichi.md)
  documents two things the mark cannot do.** A sequential discrete
