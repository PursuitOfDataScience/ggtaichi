...
rotation for a seventh and a computed comparison between the two sources
for an eighth.
...
are exported for fully manual scale control, and each layer is drawn as
one batched polygon – a 1200-cell grid renders about 15x faster than the
per-cell grob building of v0.1.0, pixel-for-pixel identically.
...
![A 12-week by 8-neighbourhood taichi grid where each glyph's eyes grow
with the gap between espresso and matcha orders, and cells where the two
agree have no eyes at all.](reference/figures/README-explicit-eye-1.png)
...
![Five taichi diagrams tilting progressively from left-leaning to
right-leaning as the difference between the two sources goes from
negative to positive.](reference/figures/README-explicit-angle-1.png)
...
draws the same statistic as a diverging heatmap, while
...
to build your own. The defaults are unchanged, so no existing figure
moves.
...
![The espresso-versus-matcha taichi grid drawn with a luminance-matched
blue and brick-red palette pair, so equal values carry equal visual
...
turns the plot into a widget whose tooltips carry both values and their
difference:
