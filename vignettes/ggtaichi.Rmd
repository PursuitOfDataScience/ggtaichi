...
Setting `eyes = TRUE` draws the classic taichi dots. Their colour and
size are fully customizable through `yin_eye_colour`, `yang_eye_colour`,
`yin_eye_size`, and `yang_eye_size`.

```{r, fig.height = 4, fig.alt="Four taichi diagrams with eyes enabled."}
one_eye <- data.frame(
  x = c(1, 2, 1, 2),
  y = c(2, 2, 1, 1),
  yin = c(3, 5, 7, 9),
...
columns are numeric or discrete (factor / character) and picks the
appropriate scale.  You can also supply a custom scale constructor
via `yin_scale` / `yang_scale`:

```{r, fig.height = 4, fig.alt="Taichi grid with discrete category fills."}
disc <- data.frame(
  x = c(1, 2, 1, 2),
  y = c(2, 2, 1, 1),
