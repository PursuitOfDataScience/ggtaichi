...
(Both waves ship together as **v0.2.0** — 0.2.0 was never released, so the
version number does not advance until it actually goes out.)
...
- **ggiraph interactivity (§2a).** Implementation sketch after the
  ...
  snapshots. Deferred so v0.2.0 stays dependency-light and CI-green.
...
- **Lifecycle badge.** Keep `experimental` through 0.2.0 (API grew again);
  revisit `stable` for 0.3.0 once the exported-geom API has survived a cycle.
