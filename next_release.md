...
- A lower-level `geom_yin_fish()` / `geom_yang_fish()` exported pair
  (they exist internally) would let power users bring any scale,
  faceting, or new_scale arrangement. Keep
  [`geom_taichi()`](https://pursuitofdatascience.github.io/ggtaichi/reference/geom_taichi.md)
...
- `geom_yin_fish()` / `geom_yang_fish()` are **not exported** (confirmed
  via `getNamespaceExports`), so the §4d decoupling genuinely needs new
  exports.
